...
fromConnectionSelSet fldTy selSet = fmap toFields $
  traverseObjectSelectionSet selSet $ \f -> do
    ...
    case _fName f of
      ...
      "pageInfo"   -> do
        selSet' <- asObjectSelectionSet fSelSet
        RS.ConnectionPageInfo <$> parsePageInfoSelectionSet fTy selSet'
      "edges"      -> do
        selSet' <- asObjectSelectionSet fSelSet
        RS.ConnectionEdges <$> parseEdgeSelectionSet fTy selSet'
...
parseEdgeSelectionSet fldTy selSet = fmap toFields $
  traverseObjectSelectionSet selSet $ \f -> do
    ...
    case _fName f of
      ...
      "node"       -> do
        selSet' <- asObjectSelectionSet fSelSet
        RS.EdgeNode <$> processTableSelectionSet fTy selSet'
